import { CELL, ZOOM_CONFIG } from '../config/config.js';
import { buildSearchItems } from '../data/data.js';
...
export class MapController {
  constructor(model, view) {
    ...
    this.startX = 0;
    this.startY = 0;
    this.searchItems = buildSearchItems();
  }
      ...
      calcBtn.addEventListener('click', () => {
        ...
        if (infoEl) {
          if (route) {
            ...
            infoEl.textContent = `Distancia: ~${distancia} metros (${route.length} celdas)`;
          } else {
  ...
  registerZoomEvents() {
    ...
    if (zoomIn) zoomIn.addEventListener('click', () => this.zoomAtCenter(ZOOM_CONFIG.FACTOR_IN));
    if (zoomOut) zoomOut.addEventListener('click', () => this.zoomAtCenter(ZOOM_CONFIG.FACTOR_OUT));
    ...
    this.view.mapWrap.addEventListener('wheel', (e) => {
      ...
      const factor = e.deltaY < 0 ? ZOOM_CONFIG.FACTOR_IN : ZOOM_CONFIG.FACTOR_OUT;
      const newScale = Math.min(ZOOM_CONFIG.MAX, Math.max(ZOOM_CONFIG.MIN, this.view.scale * factor));

      this.view.tx = mx - (mx - this.view.tx) * (newScale / this.view.scale);
      this.view.ty = my - (my - this.view.ty) * (newScale / this.view.scale);
  ...
  registerEntryPanelEvents() {
    const verEntradaBtn = document.getElementById('ver-entrada-btn');
    const entryClose = document.getElementById('entry-close');
    const entryGoBtn = document.getElementById('entry-go-btn');

    if (verEntradaBtn) {
      verEntradaBtn.addEventListener('click', () => this.view.toggleEntryPanel());
    }
    if (entryClose) {
      entryClose.addEventListener('click', () => this.view.closeEntryPanel());
    }
    if (entryGoBtn) {
      entryGoBtn.addEventListener('click', () => {
        this.model.setOrigin(3, 26);
        this.updateUI();
  ...
  executeSearch() {
    ...
    if (match) {
      this.focusOnCell(Math.floor(match.r), Math.floor(match.c));
  ...
  zoomAtCenter(factor) {
    ...
    const newScale = Math.min(ZOOM_CONFIG.MAX, Math.max(ZOOM_CONFIG.MIN, oldScale * factor));
